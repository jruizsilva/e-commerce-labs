...
const updateTotalValueCart = async (cart) => {
  ...
  let total = productsCart.map((val)=>val.totalValue).reduce((a, b) => a + b, 0);
...
const addProduct = async (req, res, next) => {
  try {
    ...
    if (!user.cartId) {
      ...
      cart = await Cart.findOne({where: {id: user.cartId}, include: {model: ProductCart}})
    }
    ...
    if (findPr) {
      ...
      await ProductCart.create({quantity, totalValue, cartId: cart.id, productId: product.id});
...
const deleteProductCart = async (req, res, next) => {
  try {
    ...
    let productCart = await ProductCart.findOne({where: {id: productCardId}});
